...
**Response**: From the above boxplots I notice that Gentoo penguins tend
to have longer flipper lengths than the other species.
...
**Response**: The boxplot provides the excellent information of
quadrants. The jitter plot is certainly helpful for raw viewing of data.
...
**Response**: The .md file is much more pleasant on the eyes, however I
still assume that which is more professional will be determined on your
specific profession?
...
**Response**: I”m personally not a fan of placing the boxplot on top of
the jitter. It seems as though it could be a misleading indicator of
density. Maybe layering them and using the `alpha` variable in `ggplot`?
...
**Response**: Mapping any arguments in the `ggplot` call sets the
arguments for all `geom_*` layers whereas, mapping them in an indivdual
`geom_*` sets the argument(s) for only that layer.
...
**Response**: It seems that the categorical variable `species` is a
large factor in these two peaks in `bill_length_mm` around 39 mm and 46
mm. you can see that the Adelie species has a bill length mean around 39
mm and both the Gentoo and Chinstrap have means around 46 mm.
